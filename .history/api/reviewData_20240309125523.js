...
const getWasThisReviewHelpful = (reviewId) => new Promise((resolve, reject) => {
  // Ensure endpoint is your Firebase URL ending with `.firebaseio.com`
  fetch(`${endpoint}/wasThisReviewHelpful.json?orderBy="$key"`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  .then((data) => {
    ...
    for (const key in data) {
      if (data.hasOwnProperty(key)) {
        ...
        for (const innerKey in reviewData) {
          if (reviewData.hasOwnProperty(innerKey)) {
            const record = reviewData[innerKey];
