...
const getWasThisReviewHelpful = (reviewId) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/wasThisReviewHelpful.json?orderBy="reviewId"&equalTo="${encodeURIComponent(reviewId)}"`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  .then((data) => {
    ...
    object. (let key in data) {
      if (data.hasOwnProperty(key)) {
        let value = data[key];
        for (let nestedKey in value) {
          if (value.hasOwnProperty(nestedKey) && value[nestedKey].reviewId === reviewId) {
            ratings.push(value[nestedKey].rating);
          }
        }
      }
    }
