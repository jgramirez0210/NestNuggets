...
const createReviewRating = (payload) => new Promise((resolve, reject) => {
  ...
  fetch(`${endpoint}/reviewRating.json`, {
    ...
    .then((data) => {
      const firebaseKey = data.name;
      const reviewWithKey = { ...payload, firebaseKey };
      return fetch(`${endpoint}/reviewRating/${firebaseKey}.json`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
export {
  getReview, createReview, updateReview, createReviewRating, getReviewByUser, deleteReview, getSingleReview, viewReviewDetails, getReviewById,
};
