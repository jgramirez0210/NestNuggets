...
const updateWasThisHelpfulReviewRating = async (firebaseKey, newRating) => {
  console.warn('Update firebaseKey:', firebaseKey); // Log the firebaseKey

  // Check if the firebaseKey exists
  const response = await fetch(`${endpoint}/wasThisReviewHelpful/${firebaseKey}.json`);
  const data = await response.json();

  if (data === null) {
    console.error('firebaseKey does not exist:', firebaseKey);
    return;
  }
