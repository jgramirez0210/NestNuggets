...
const getWasThisReviewHelpful = (reviewId) => new Promise((resolve, reject) => {
  ...
  .then((data) => {
    ...
    // Since the reviewId is nested, we need to filter the results manually
    const ratings = Object.values(data).reduce((acc, item) => {
      const reviewData = Object.values(item).find(review => review.reviewId === reviewId);
      if (reviewData) {
        acc.push(reviewData.rating);
      }
      return acc;
    }, []);
