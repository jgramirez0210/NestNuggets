...
const createWasThisHelpfulReviewRating = (payload) => new Promise((resolve, reject) => {
  // Convert firebaseKey to a number
  const payloadWithNumberKey = {
    ...payload,
    firebaseKey: Number(payload.firebaseKey),
  };
  ...
  fetch(`${endpoint}/wasThisReviewHelpful.json`, {
    ...
    body: JSON.stringify(payloadWithNumberKey),
  })
    .then((response) => response.json())
    .then((data) => {
      const firebaseKey = data.name;
      const reviewWithKey = { ...payloadWithNumberKey, firebaseKey };
