...
const createWasThisHelpfulReviewRating = (payload) => new Promise((resolve, reject) => {
  ...
  fetch(`${endpoint}/wasThisHelpful.json`, {
    ...
    .then((data) => {
      const firebaseKey = data.name;
      const reviewWithKey = { ...payload, firebaseKey };
      return fetch(`${endpoint}/wasThisReHelpful/${firebaseKey}.json`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
export {
  getReview, createReview, updateReview, createWasThisHelpfulReviewRating, getReviewByUser, deleteReview, getSingleReview, viewReviewDetails, getWasThisHelpfulReviewById,
};
