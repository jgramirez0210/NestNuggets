...
const createWasThisHelpfulReviewRating = ({ reviewId, ...rest }) => new Promise((resolve, reject) => fetch(`${endpoint}/wasThisReviewHelpful/${reviewId}.json`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  ...
  .then((response) => response.json())
  .then((data) => {
    console.warn('New document created with ID:', data.name);
    resolve(data.name);
  })
  .catch(reject));
// UPDATE WAS THIS RATING HELPFUL
const updateWasThisHelpfulReviewRating = (payload) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/wasThisReviewHelpful/${payload.reviewId}/${payload.uid}.json`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
...
const checkIfRatingExists = (reviewId, uid) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/wasThisReviewHelpful.json?orderBy="reviewId"&equalTo="${reviewId}"&print=pretty`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
...
export {
  getReview, createReview, updateReview, getReviewByUser, deleteReview, getSingleReview, createWasThisHelpfulReviewRating, updateWasThisHelpfulReviewRating, getWasThisHelpfulReviewRating,checkIfRatingExists, getCurrentRating,
};
