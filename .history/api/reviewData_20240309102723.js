...
// Assuming `endpoint` is defined elsewhere in your code
const getWasThisReviewHelpful = (reviewId) => new Promise((resolve, reject) => {
  console.warn(`Fetching review helpfulness data for review ID: ${reviewId}`);
  fetch(`${endpoint}/wasThisReviewHelpful/${reviewId}.json`, {
    ...
    .then((data) => {
      ...
      const wasThisHelpfulReviewById = data ? Object.values(data) : [];
      console.warn('Processed data:', wasThisHelpfulReviewById);
      resolve(wasThisHelpfulReviewById);
    })
    .catch((error) => {
