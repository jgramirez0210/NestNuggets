...
const createWasThisHelpfulReviewRating = (payload) => new Promise((resolve, reject) => {
  const reviewId = payload.reviewId; // Get the current reviewId from the payload
  ...
  fetch(`${endpoint}/wasThisReviewHelpful.json`, {
    ...
    .then((response) => response.json())
    .then((data) => {
      // Use the current reviewId instead of the firebaseKey
      return fetch(`${endpoint}/wasThisReviewHelpful/${reviewId}.json`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ...payload, firebaseKey: data.name }),
      });
    })
    .then((response) => response.json())
    .then(resolve)
...
// GET CURRENT RATINGS
const getCurrentRating = (reviewId) => new Promise((resolve, reject) => {
  fetch(`${endpoint}/wasThisReviewHelpful.json`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
