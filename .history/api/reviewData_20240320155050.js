...
const createWasThisHelpfulReviewRating = ({ reviewId, ...rest }) => new Promise((resolve, reject) => fetch(`${endpoint}/wasThisReviewHelpful.json`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  ...
  .then((response) => response.json())
  .then((data) => fetch(`${endpoint}/wasThisReviewHelpful/${reviewId}.json`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
     },
      body: JSON.stringify({ ...rest, firebaseKey: data.name }),
    }))
  .then((response) => response.json())
  .then(resolve)
  .catch(reject));
