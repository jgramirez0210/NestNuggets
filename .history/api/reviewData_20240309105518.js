...
const getWasThisReviewHelpful = (reviewId) => new Promise((resolve, reject) => {
  axios.get(`${endpoint}/wasThisReviewHelpful.json?orderBy="reviewId"&equalTo="${reviewId}"`)
    .then((response) => {
      const helpfulReviews = response.data;
      const reviews = [];
      if (helpfulReviews) {
        Object.keys(helpfulReviews).forEach((fbKey) => {
          reviews.push(helpfulReviews[fbKey]);
        });
      }
      resolve(reviews);
    })
