...
const StarRating = ({ reviews }) => {
  ...
  useEffect(() => {
    const matchingReview = reviews.find((review) => review.category === category);
    if (matchingReview) {
      setFirebaseKey(matchingReview.firebaseKey);
    }
  }, [category, reviews]);
  ...
  const handleRating = (value, newCategory) => {
    ...
    // Call updateRating after state has been updated
