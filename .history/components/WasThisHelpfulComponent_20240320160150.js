...
const WasThisReviewHelpful = ({ firebaseKey, reviews, initialKey }) => {
  ...
  const handleRating = async (ratingValue) => {
    ...
    console.log('Before checkIfRatingExists');
    const exists = await checkIfRatingExists(firebaseKey, user.uid);
    console.log('After checkIfRatingExists');

    if (exists) {
      console.log('Before updateWasThisHelpfulReviewRating');
      await updateWasThisHelpfulReviewRating(payload);
      console.log('After updateWasThisHelpfulReviewRating');
    } else {
      console.log('Before createWasThisHelpfulReviewRating');
      await createWasThisHelpfulReviewRating(payload);
      console.log('After createWasThisHelpfulReviewRating');
    }
  };
  return (
