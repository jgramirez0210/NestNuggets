...
const WasThisReviewHelpful = ({ initialKey, userId }) => {
  ...
  const handleRating = async (ratingValue) => {
    ...
    if (!ratingExists || !hasRated) {
      ...
      if (!initialFirebaseKey) {Unexpected if as the only statement in an else block.Unexpected if as the only statement in an else block.console.error('initialFirebaseKey is undefined');
        return;
      }
    }
  };
