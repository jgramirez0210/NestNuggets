...
import {
  createWasThisHelpfulReviewRating, updateWasThisHelpfulReviewRating, getWasThisReviewHelpful, getAllRatings,
} from '../api/reviewData';
...
const WasThisReviewHelpful = ({ firebaseKey: initialFirebaseKey, reviews = [] }) => {
  ...
  return (
    <div className="helpful-rating" style={{ flexDirection: 'column' }}>
      <p>Was this helpful?</p>
      {[...Array(5)].map((star, i) => {
        const ratingValue = i + 1;
        const inputId = `helpfulRating-${ratingValue}`;
        return (
          <div className="wasThisHelpful" key={inputId} style={{ display: 'flex', flexDirection: 'row' }}>
            <input
              type="radio"
              id={inputId}
              name="helpfulRating"
