...
import { createWasThisReviewHelpfulReviewRating, updateWasThisHelpfulReviewRating } from '../api/reviewData';
...
const WasThisReviewHelpful = ({ firebaseKey: initialFirebaseKey, reviews = [] }) => {
  ...
  const handleRating = (ratingValue) => {
    ...
    if (!hasRated) { // If the user has not rated yet
      createWasThisReviewHelpfulReviewRating(payload);
      setHasRated(true);
    } else { // If the user has already rated
