...
const StarRating = ({ reviews }) => {
  ...
  const handleRating = (value, category) => {
    let overviewRating = currentOverviewRating; // replace with current value from Firebase
    let safetyRating = currentSafetyRating; // replace with current value from Firebase
    let managementRating = currentManagementRating; // replace with current value from Firebase

    switch (category) {
      case 'overview':
        overviewRating = value;
        break;
      case 'safety':
        safetyRating = value;
        break;
    ...
    reviews.forEach((review) => {
      if (review.category === category) {
        updateRating(review.firebaseKey, overviewRating, safetyRating, managementRating);
      }
    });
  };
  return (
