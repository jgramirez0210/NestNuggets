...
const WasThisReviewHelpful = ({ firebaseKey, reviews, initialKey }) => {
  ...
  import checkIfRatingExists from './checkIfRatingExists.js';
  import { createWasThisHelpfulReviewRating, updateWasThisHelpfulReviewRating } from '../api/reviewData.js';

  const handleRating = async (reviewId, uid, newRating) => {
    const ratingExists = await checkIfRatingExists(reviewId, uid);

    if (ratingExists) {
      // If a rating exists for the current user, update it
      await updateWasThisHelpfulReviewRating(reviewId, uid, newRating);
    } else {
      // If no rating exists for the current user, create a new one
      await createWasThisHelpfulReviewRating(reviewId, uid, newRating);
    }
  };
  return (
