  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebaseKey)
      .then((data) => {
        ...
        const nestedRatings = Object.values(data).map((nestedObj) => Object.values(nestedObj).map((item) => parseFloat(item.rating)));
        console.log('Nested ratings:', nestedRatings);
  
        // Flatten the nested array of ratings and filter out NaN values
        const ratings = nestedRatings.flat().filter((rating) => !Number.isNaN(rating));
