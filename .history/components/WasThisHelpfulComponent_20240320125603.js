...
const WasThisReviewHelpful = ({ firebaseKey, reviews, initialKey }) => {
  ...
  const handleRating = async (ratingValue) => {
    ...
    if (doesRatingExist) {
      ...
      const createWasThisHelpfulReviewRating = async (payload) => {
        const db = firebase.database();
        const newRatingRef = db.ref('ratings').push(); // Generate a unique ID for the new rating
        await newRatingRef.set(payload); // Set the data for the new rating
      };
  };
