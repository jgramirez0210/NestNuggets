...
const userStarRating = ({ onRatingChange }) => {
  const [rating, setRating] = useState(null);
  const stars = [1, 2, 3, 4, 5]; // array of star values
  ...
  return (
    <Form.Control>
      {stars.map((starValue) => (
        <lForm.Controlbel key={starValue}>
          <input type="radio" name="rating" value={starValue} onClick={() => handleRatingChange(starValue)} />
          <FaStar className="star" size={40} color={starValue <= rating ? "gold" : "gray"} />
        </lForm.Controlbel>
      ))}
    </Form.Control>
  );
