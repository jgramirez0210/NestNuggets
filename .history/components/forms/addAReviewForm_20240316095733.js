...
function AddAReviewForm({ obj, firebaseKey }) {
  ...
  useEffect(() => {
    if (firebaseKey) { // Only fetch the review details if firebaseKey is not null
      viewReviewDetails(firebaseKey).then((data) => {
        console.warn('viewReviewDetails Response:', data);
        setFormInput(data); // Set the form state with the review details
      });
    }
  }, [firebaseKey]);
...
AddAReviewForm.propTypes = {
  ...
  firebaseKey: PropTypes.string, // Validate firebaseKey separately
};
