...
import {
  createReview, updateReview, getWasThisHelpfulReviewById,
} from '../../api/reviewData';
import WasThisHelpfulComponent from '../StarRatingComponent';
...
function AddAReviewForm({ obj }) {
  ...
  useEffect(() => {
    getWasThisHelpfulReviewById(user.uid).then((reviewRatings) => {
      setReview(reviewRatings);
      console.warn('reviewRating', reviewRatings);
    });
  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      {/* RATING */}
      <FloatingLabel>
        <StarRating firebaseKey={formInput.reviewId} reviews={[{ category: 'overall' }, { category: 'safety' }, { category: 'management' }]} />
      </FloatingLabel>
      {/* RENTAL DURATION */}
