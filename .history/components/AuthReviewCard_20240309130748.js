...
function AuthReviewCard({
  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebaseKey)
      .then((data) => {
        // We have to dig one level deeper due to the Firebase keys
        const nestedRatings = Object.values(data).map((nestedObject) => Object.values(nestedObject).map((item) => parseFloat(item.rating))
        );
  
        // Flatten the array of arrays into a single array of ratings
        const ratings = [].concat(...nestedRatings).filter((rating) => !isNaN(rating));
  
        // Now we can calculate the sum of the ratings
        const sum = ratings.reduce((a, b) => a + b, 0);
  
        // Calculate the average rating
        const avg = ratings.length > 0 ? sum / ratings.length : 0;
  
        // Update state with the calculated average
