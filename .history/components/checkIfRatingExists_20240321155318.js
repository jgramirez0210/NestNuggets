...
const checkIfRatingExists = async (reviewId, uid) => {
  ...
  if (Array.isArray(ratings)) {
    ...
    ratingExists = ratings.some((rating) => {
      console.warn('Comparing reviewId:', rating.reviewId, 'with input reviewId:', reviewId);
      console.warn('Comparing uid:', rating.uid, 'with input uid:', uid);
      return rating.reviewId === reviewId && rating.uid === uid;
    });
