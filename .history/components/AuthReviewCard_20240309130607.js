...
function AuthReviewCard({
  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebaseKey)
      .then((ratingObjects) => {
        // Convert each rating to a number and filter out non-numeric values
        const validRatings = ratingObjects.map((item) => parseFloat(item.rating)).filter((rating) => !Number.isNaN(rating));
  
        // Calculate the sum of the valid ratings
        const sum = validRatings.reduce((a, b) => a + b, 0);
  
        // Calculate the average from the sum and the count of valid ratings
        const avg = validRatings.length > 0 ? sum / validRatings.length : 0;
  
        // Update state with the average rating
