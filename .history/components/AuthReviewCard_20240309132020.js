...
function AuthReviewCard({
  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebreaseKey)
      .then((data) => {
        // We expect 'data' to be an object with keys that are Firebase-generated IDs
        // Each of these keys corresponds to an object that represents a "review group",
        // where the actual reviews are nested objects within.
  
        const allRatings = [];
        Object.keys(data).forEach((topLevelKey) => {
          const reviewGroup = data[topLevelKey];
          Object.keys(reviewGroup).forEach((secondLevelKey) => {
            const reviewDetails = reviewGroup[secondLevelKey];
            // Checking the reviewId matches and that rating is a number before pushing
            if (reviewDetails.reviewId === reviewObj.firebaseKey && !isNaN(Number(reviewDetails.rating))) {
              allRatings.push(Number(reviewDetails.rating));
            }
          });
        });
  
        console.log('All Ratings:', allRatings); // Should log an array of numbers
        ...
        console.log('Average rating:', averageRating); // Should log the average rating
        setHelpfulReviews(averageRating); // Sets the state to the average rating
      })
      .catch((error) => {
        console.error('Error fetching review helpful data:', error); // Error handling
      });
  }, [reviewObj]);
