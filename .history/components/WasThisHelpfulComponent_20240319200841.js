...
const WasThisReviewHelpful = ({ initialKey }) => {
  ...
  useEffect(() => {
    ...
    console.warn('initialFirebaseKey', initialFirebaseKey);
  }, [initialFirebaseKey, initialKey]);
  ...
  const handleRating = async (ratingValue) => {
    ...
    if (!ratingExists || !hasRated) {
      const response = await createWasThisHelpfulReviewRating(payload); // Save the response
      // console.log(response); // Log the response
      localStorage.setItem(`hasRated-${initialFirebaseKey}-${uid}`, JSON.stringify(true));
    } else if (!initialFirebaseKey) {
...
WasThisReviewHelpful.propTypes = {
  ...
  initialKey: PropTypes.string, // Add this line
};

WasThisReviewHelpful.defaultProps = {
  reviews: [],
  firebaseKey: '',
  initialKey: '', // Add this line
};
