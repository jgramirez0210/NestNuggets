...
function AuthReviewCard({
  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebaseKey)
      .then((data) => {
        console.warn('data from getWasThiReviewHelpful', reviewObj.firebaseKey);
        // Extract the rating from each item and calculate the average
        const ratings = Object.values(data).map((item) => item.rating);
