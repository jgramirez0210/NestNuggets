...
const WasThisReviewHelpful = ({ firebaseKey, reviews, initialKey }) => {
  ...
  const handleRating = async (ratingValue) => {
    ...
    const payload = {
      ...
      uid,
      uniqueId: Date.now(), // Add a unique identifier to the payload
    };
    ...
    // Always create a new entry, regardless of whether a rating already exists
    ...
    localStorage.setItem(`hasRated-${firebaseKey}-${uid}`, JSON.stringify(true));
      ...
      console.error('firebaseKey is undefined');
    }
  };
  return (
