...
const WasThisReviewHelpful = ({ firebaseKey, reviews, initialKey }) => {
  ...
  const handleRating = async (newRating) => {
    ...
    const firebaseKeyExists = async (key) => fetch(`${endpoint}/wasThisReviewHelpful/${key}.json`)
    ...
    if (ratingFirebaseKey && await firebaseKeyExists(ratingFirebaseKey)) {
      ...
      const newRatingId = generateNewRatingId(); // You'll need to implement this function
      createWasThisHelpfulReviewRating({ reviewId: firebaseKey, uid, rating: newRating, id: newRatingId });
    }
  };
  return (
