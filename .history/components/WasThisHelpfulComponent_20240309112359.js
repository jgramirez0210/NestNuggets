...
const WasThisReviewHelpful = ({ firebaseKey: initialFirebaseKey, reviews = [] }) => {
  ...
  const handleRating = async (ratingValue) => {
    ...
    const ratings = await getWasThisReviewHelpful(initialFirebaseKey);
    console.warn(ratings);

    // Check if a rating from the same uid already exists
    const existingRating = ratings.find((rating) => rating.uid === uid);

    if (existingRating) {
      // If a rating from the same uid exists, update it
      updateWasThisHelpfulReviewRating(existingRating.firebaseKey, payload);
    } else {
      // If no rating from the same uid exists, create a new one
