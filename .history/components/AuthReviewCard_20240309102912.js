...
function AuthReviewCard({ reviewObj, onDashboard, onUpdate }) {
  ...
  useEffect(() => {
    const fetchAverageRating = async () => {
      try {
        const wasReviewHelpfulData = await getWasThisReviewHelpful(reviewObj.rating);
        console.warn('wasThisReviewHelpful', wasReviewHelpfulData);
        if (wasReviewHelpfulData.length > 0) {
          ...
          if (validRatings.length > 0) {
            ...
            console.warn('reviewAverageRating', reviewAverageRating);
  ...
  return (
    <Card style={{ width: '30rem', margin: '10px' }}>
      ...
      <Card.Body>
        ...
        <p>Average Rating: {reviewAverageRating? reviewAverageRating.toFixed(2) : null}</p>
        {onDashboard && (
          <>
