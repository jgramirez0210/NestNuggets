...
function AuthReviewCard({
  ...
  useEffect(() => {
    getWasThisReviewHelpful(reviewObj.firebaseKey)
      .then((ratingObjects) => {
        // Assuming each item is an object with a 'rating' property
        const ratings = ratingObjects.map((item) => {
          // Convert rating to number if it's a string
          const rating = parseFloat(item.rating);
          return rating; // make sure this is a number
        });
        // Filter out any NaN values from the ratings
