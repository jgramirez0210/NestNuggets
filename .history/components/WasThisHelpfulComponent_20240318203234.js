...
const WasThisReviewHelpful = ({firebaseKey, firebaseKey: initialFirebaseKey, reviews = [] }) => {
  ...
  useEffect(() => {
    // Get hasRated from localStorage
    const storedHasRated = localStorage.getItem(`hasRated-${firebaseKey}`);
    const hasRatedFromStorage = storedHasRated ? JSON.parse(storedHasRated) : false;

    // Log the hasRated state
    console.warn('Has Rated:', hasRatedFromStorage);
  }, [firebaseKey]); // Add firebaseKey to the dependency array
